...
export class GeminiImageGen extends BaseImageGen {
  ...
  async generate(prompt, opts = {}) {
    ...
    cliLogger.log('image', `Gemini image generate: model=${this.model}, aspect=${aspectRatio}, res=${resolution}`);
    const _t0 = Date.now();
    ...
    try {
      ...
      const _elapsed = Date.now() - _t0;
      cliLogger.log('image', `Gemini image generate completed in ${_elapsed}ms`);
      ...
      cliLogger.log('image', `Gemini image generate FAILED: ${err.message}`);
      throw err;
    }
  }
}
...
export class GeminiVideoGen extends BaseVideoGen {
  ...
  async generate(prompt, opts = {}) {
    ...
    cliLogger.log('video', `Veo generate: model=${this.model}, aspect=${aspectRatio}, duration=${duration}s, res=${resolution}`);
    const _t0 = Date.now();
    ...
    try {
      ...
      const data = await res.json();
      const _elapsed = Date.now() - _t0;
      cliLogger.log('video', `Veo generate submitted in ${_elapsed}ms: ${data.name}`);
      ...
      cliLogger.log('video', `Veo generate FAILED: ${err.message}`);
      throw err;
    }
  }
  ...
  async getStatus(jobId, opts = {}) {
    try {
      ...
      cliLogger.log('video', `Veo status FAILED: ${err.message}`);
      throw err;
    }
  }
}
