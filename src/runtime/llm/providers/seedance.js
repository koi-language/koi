...
export class SeedanceVideoGen extends BaseVideoGen {
  ...
  async generate(prompt, opts = {}) {
    ...
    channel.log('video', `Seedance video generate: model=${this.model}, aspect=${aspectRatio}, duration=${duration}s, res=${resolution}`);
    const _t0 = Date.now();
    ...
    try {
      ...
      channel.log('video', `Seedance video task submitted in ${_elapsed}ms: ${data.data?.task_id}`);
      ...
      channel.log('video', `Seedance video generate FAILED: ${err.message}`);
      throw err;
    }
  }
  ...
  async getStatus(jobId, opts = {}) {
    try {
      ...
      channel.log('video', `Seedance video status FAILED: ${err.message}`);
      throw err;
    }
  }
}
