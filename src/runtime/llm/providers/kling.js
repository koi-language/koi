...
export class KlingVideoGen extends BaseVideoGen {
  ...
  async generate(prompt, opts = {}) {
    ...
    channel.log('video', `Kling ${endpoint}: model=${this.model}, aspect=${aspectRatio}, duration=${duration}s`);
    const _t0 = Date.now();
    ...
    try {
      ...
      channel.log('video', `Kling video task submitted in ${_elapsed}ms: ${data.data?.task_id}`);
      ...
      channel.log('video', `Kling video generate FAILED: ${err.message}`);
      throw err;
    }
  }
  ...
  async getStatus(jobId, opts = {}) {
    try {
      ...
      channel.log('video', `Kling video status FAILED: ${err.message}`);
      throw err;
    }
  }
}
