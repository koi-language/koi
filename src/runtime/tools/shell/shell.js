...
async function _loadPty() {
  ...
  try {
    ...
    channel.log('shell', 'node-pty loaded ✓');
    ...
    channel.log('shell', `node-pty unavailable, using spawn fallback`);
  }
}
...
function _extractSpawnHelperIfNeeded() {
  try {
    ...
    if (!fs.existsSync(tempHelper)) {
      ...
      channel.log('shell', `spawn-helper extracted to ${tempHelper}`);
    }
    ...
    channel.log('shell', `spawn-helper patched → ${tempHelper}`);
  } catch (e) {
    channel.log('shell', `spawn-helper extraction skipped: ${e.message}`);
  }
}
 ...
 * Set inside _drainPermQueue while awaiting channel.select; cleared when done.
...
export function cancelActivePermission() {
  _activeSelectCancel?.(); // resolve the active channel.select with null (= denied)
...
async function _drainPermQueue() {
  ...
  while (_permQueue.length > 0) {
    ...
    // Race the channel.select against a cancel token so that if the delegate times out
  ...
  async * execute(action, agent) {
    ...
    if (background) {
      ...
      bgChild.on('close', (code) => {
        ...
        channel.log('background', `PID ${bgChild.pid} exited code=${code ?? 'signal'} cmd="${cmdPreview}"`);
    ...
    const _onClose = (code) => {
      ...
      if (!isSilent) channel.clearProgress();
      isClosed = true;
      closeCode = code;
      _notify();
    };
        ...
        if (!isClosed) {
          ...
          if (_inputNeededInfo && !isClosed) {
            ...
            if (agentAnswer != null && !isClosed && proc) {
              channel.log('shell', `[agent-input] writing answer to stdin`);
