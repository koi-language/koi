...
export default {
  ...
  description:
    'Append a clip to a timeline. The clip object: { track ("V1"/"V2"/"A1"/…), path (absolute), startMs, durationMs, sourceInMs?, sourceTotalMs?, linkId? }. ' +
    'Use the same linkId on a V-clip and an A-clip to keep them paired (so move/trim affect both). ' +
  ...
  async execute(params) {
    try {
      const tl = addClip(params.id, params.clip);
