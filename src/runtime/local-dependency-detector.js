...
export function addManualDependency(projectDir, depPath, name, reason) {
  ...
  cliLogger.log('dep-detector', `Manual dependency added: ${resolved} (${reason || 'no reason'})`);
...
export function seedDependenciesFile(projectDir) {
  ...
  if (added > 0 || existing.length > 0) {
    ...
    if (added > 0) {
      cliLogger.log('dep-detector', `Seeded ${added} new dependencies into dependencies.json`);
    }
  }
}
        ...
        try {
          ...
          for (const dep of deps) {
            ...
            if (resolved !== projectDir && fs.existsSync(resolved)) {
              found.add(resolved);
              cliLogger.log('dep-detector', `[${detector.name}] ${path.basename(filePath)} → ${resolved}`);
            }
          }
        } catch (err) {
          cliLogger.log('dep-detector', `[${detector.name}] Error processing ${filePath}: ${err.message}`);
        }
      }
    }
  }
...
export function detectAllLocalDependencies(projectDir, maxDepth = 3) {
  ...
  walk(projectDir, 0);
  cliLogger.log('dep-detector', `Total local dependencies for ${path.basename(projectDir)}: ${all.size} projects`);
  return [...all];
}
