...
import { getPlatform, getLastFrameInfo, invalidateElementsCache, getCachedElements, getLatestElements } from '../mobile/platform.js';
import { findElementByLabel, getElementCenter, formatElementsSummary } from '../mobile/element-matching.js';
import { cellToCoordinates, isValidCell } from '../mobile/grid-overlay.js';
...
export default {
  ...
  thinkingHint: (action) => {
    ...
    if (action.cell || action.x != null) {
      try {
        const { getLatestElements } = require('../mobile/platform.js');
        const latest = getLatestElements();
        if (latest?.elements?.length > 0) {
          const { cellToCoordinates } = require('../mobile/grid-overlay.js');
          const { getLastFrameInfo } = require('../mobile/platform.js');
