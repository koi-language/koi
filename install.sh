...
check_dependencies() {
  ...
  if ! version_compare "${NODE_VERSION}" "${MIN_NODE_VERSION}"
  then
  ...
  # Check git (verify it actually works, not just that the binary exists)
  ...
  GIT_VERSION_OUTPUT=$(git --version 2>&1) || {
    log_error "git is installed but not functional"
    if [[ "${OS}" == "macos" ]]
    then
      log_error "You may need to accept the Xcode license agreement:"
      echo ""
      echo "  sudo xcodebuild -license accept"
      echo ""
      echo "Or install standalone git:"
      echo "  brew install git"
    fi
    exit 1
  }

  GIT_VERSION_NUM=$(echo "${GIT_VERSION_OUTPUT}" | awk '{print $3}')
  if [[ -z "${GIT_VERSION_NUM}" ]]
  then
    log_error "git is installed but not working correctly"
    if [[ "${OS}" == "macos" ]]
    then
      log_error "You may need to accept the Xcode license agreement:"
      echo ""
  ...
  log_success "git ${GIT_VERSION_NUM} found"
}
...
install_koi() {
  ...
  if ! git clone --depth 1 "${REPO_URL}" "${INSTALL_DIR}" 2>&1 | sed 's/^/  /'
  then
    log_error "Failed to clone repository"
    exit 1
  fi

  # Verify clone succeeded (check for package.json as a canary)
  if [[ ! -f "${INSTALL_DIR}/package.json" ]]
  then
    log_error "Repository clone appears incomplete (package.json not found)"
    log_error "This may be caused by a git or network issue"
    exit 1
  fi
